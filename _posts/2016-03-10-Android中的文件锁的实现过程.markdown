...
##  需求
项目中有个需求是文件加锁，开始需求比较急，同事是通过移动文件实现的，移动到应用的私有缓存目录，这样就只有我们自己可以访问了。但这样有些问题，一是清除缓存后所有文件都没有了，而是每次加锁费时较长。
...
文件加密后被我们重命名在原目录了，这时候要把加密后的文件路径记录在我们的数据库里面，文件再重命名为已点开头，这样自动识别为隐藏文件就不容易被他人访问到了。针对视频和图片还需要让媒体库更新，不然还会看到一个打不开的文件，方法是发送一个更新媒体库的广播，代码如下：
...
我们的图片加载用的Picasso,可以通过RequestHandler重写load方法加载加密图片了。
